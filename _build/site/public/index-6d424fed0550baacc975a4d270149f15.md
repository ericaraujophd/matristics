...
|-------|----------|-----|----------|
| Claude (claude-opus-4, claude-sonnet-4) | Anthropic | Anthropic API | {doc}`claude` |
| GPT-4o, GPT-4 | OpenAI | OpenAI API | {doc}`openai` |
| Gemini 1.5 Pro / 2.0 | Google | Google AI API | {doc}`gemini` |
| Grok-2 | xAI | xAI API | {doc}`grok` |

## Batch Collection

Once individual API connections are set up, the {doc}`batch-collection` notebook runs the full question bank across all models systematically, handling rate limits, logging errors, and saving responses in a consistent JSON format ready for annotation.

## Before You Begin

See {doc}`setup` for environment setup, API key management, and the response storage format.
