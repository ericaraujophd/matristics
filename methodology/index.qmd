...
- **No formal calibration round.** The rubric was refined collaboratively during design, but there was no separate pilot annotation with an agreement gate before production scoring. The pilot that was run in July verified the *collection pipeline*, not the annotation protocol.
...
An earlier version of this methodology set a single gate — production annotation may proceed only when α ≥ 0.60 on every dimension. The first round showed that gate to be unusable as stated: it would reject a dataset in which annotators agreed exactly three-quarters of the time, for a reason that reflects model performance rather than annotator quality. The criterion has been revised accordingly:

1. **Alpha is reported per dimension, always**, as a diagnostic — never omitted when inconvenient.
2. **Alpha governs the gate only when the score distribution can support it** — operationally, when no single scale point holds more than 70% of the ratings on that dimension. Where that holds, the α ≥ 0.60 threshold applies.
3. **Where the distribution is concentrated**, as in the first round, reliability is judged instead on raw agreement: exact agreement of at least 65% and within-one-point agreement of at least 85%, with every disagreement of two points or more adjudicated individually.
4. **Reporting follows reliability**. Differences between models smaller than the mean inter-annotator difference on that dimension are reported as provisional, not as findings.

This is a revision made in light of evidence, and it is stated as such rather than presented as the original plan. Expanding the question bank to figures with sparser documentation should widen the score distribution and return alpha to being informative under criterion 2.
