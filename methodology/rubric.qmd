...
The rubric design follows the G-Eval framework of [@liu2023geval], which uses explicit scoring criteria per dimension to improve alignment between annotators and reduce reliance on annotator background knowledge alone. The seven dimensions were developed through iterative piloting informed by the broader evaluation literature ([@zheng2023judging]).
...
::: {.callout-warning title="Note on evasive non-answers"}
